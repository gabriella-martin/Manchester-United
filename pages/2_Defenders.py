...
#connecting to database & getting defenders names

name_list = []

with psycopg2.connect(host=ENDPOINT, user=USER, password=PASSWORD, dbname=DATABASE, port=PORT) as conn:
    with conn.cursor() as cur:
        cur.execute('''SELECT name from players WHERE position ='DF' AND ninteys >1 AND club ='Manchester United';''')
...
@st.cache_data
def united_comparison(first_choice, second_choice):
